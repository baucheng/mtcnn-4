...
def drawBoxes(im, boxes, imgname, bbox_gt):
    x1 = boxes[:,0]
    y1 = boxes[:,1]
    x2 = boxes[:,2]
    y2 = boxes[:,3]
...
def initFaceDetector():
    minsize = 20
    caffe_model_path = "/home/dkk/projects/DuinoDu_mtcnn/model"
...
def drawPoints(im, points):
    ...
    for i in range(points.shape[0]):
        for j in range(points.shape[1] / 2):
            #print points[i,j*2]
            #print points[i,j*2+1]
            cv2.circle(im, (int(points[i,j]),int(points[i,j+5])),2,(0,0,255),2)
    return im

def main():
    image_path = "./afw_image"
    #imglistfile = "./file.txt"
    imglistfile = "./list.txt"
    #imglistfile = "/home/duino/iactive/mtcnn/all.txt"
    #imglistfile = "./imglist.txt"
    #imglistfile = "/home/duino/iactive/mtcnn/file_n.txt"
    #imglistfile = "/home/dkk/projects/caffe_mtcnn/celeba/mtcnn_train_label.txt"
    #root_path = "/home/dkk/projects/caffe_mtcnn/celeba/img_celeba"
    minsize = 80
    ...
    with open(imglistfile, 'r') as f:
        lines = f.readlines()
    count = 0
    bbox_gt = open("bbox_pre_DuinoDu.txt", 'w')
    for imgpath1 in lines:
        imgname = imgpath1.split('\n')[0]
...
#            cv2.rectangle(img, (int(boundingboxes[i][0]), int(boundingboxes[i][1])), (int(boundingboxes[i][2]), int(boundingboxes[i][3])), (0,255,0), 1)    
        ...
        for i in range(5):
            print points[0, i+5]
            print imgpath1.split(' ')[i + 6]
            points[0,i] = imgpath1.split(' ')[2*i + 5]
            points[0,i+5] = imgpath1.split(' ')[2*i + 6]
        """
        #print points

        img = drawBoxes(img, boundingboxes, imgname, bbox_gt)
        #img = drawPoints(img, points)
        save_path = os.path.join('./results', "%s.jpg" % imgname)
        # cv2.imshow('img', img)
