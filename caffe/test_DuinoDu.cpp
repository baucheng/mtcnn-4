...
void cvMat2Blob(cv::Mat src1, Blob<float>* input_layer,
                const float* mean, float scale) {
  cv::Mat src;
  cv::cvtColor(src1, src, cv::COLOR_BGR2RGB);
  int H = src.rows;
  int W = src.cols;
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    cout << "Usage for example: *.bin path_list" << endl;
    return -1;
  }
