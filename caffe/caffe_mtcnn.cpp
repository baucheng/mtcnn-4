  ...
  for(int h = 0; h < H; ++h) {
    for(int w = 0; w < W; ++w) {
      pData[h * W + w + 0 * offset] =
          (float(src.at<cv::Vec3b>(h, w)[0]) - mean[0]) * scale;
      pData[h * W + w + 1 * offset] =
          (float(src.at<cv::Vec3b>(h, w)[1]) - mean[1]) * scale;
      pData[h * W + w + 2 * offset] =
          (float(src.at<cv::Vec3b>(h, w)[2]) - mean[2]) * scale;
    }
  }
}
      ...
      for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
          ...
          if (conf >= confs[0]) {
            ...
            float x1 = box->cpu_data()[i + j * width_box + plane_size * 0];
            float y1 = box->cpu_data()[i + j * width_box + plane_size * 1];
            float x2 = box->cpu_data()[i + j * width_box + plane_size * 2];
            float y2 = box->cpu_data()[i + j * width_box + plane_size * 3];
